...
def sanitize_text(text: str) -> str:
    ...
    # Remove any remaining special characters that could break JSON/SQL
    # Keep only: letters, numbers, spaces, basic punctuation, and newlines
    text = re.sub(r'[^\w\s.,!?;:()\-\'"\n/@$%&*+=]', '', text)
...
class ChatbotMemory:
    ...
    def add_custom_info(self, topic: str, information: str):
        """Add a new custom info record that matches existing JSON structure"""
        try:
            # Always reload latest custom info from file (in case multiple admins are adding)
            self.custom_info = self.load_custom_info()

            # Create a safe slug key from the topic name
            # Example: "Vice President for Research..." → "vice_president_for_research"
            slug = re.sub(r'[^a-z0-9]+', '_', topic.strip().lower()).strip('_')

            # If a key with this slug already exists, append an incremental suffix (like _2, _3)
            base_slug = slug
            counter = 1
            while slug in self.custom_info:
                slug = f"{base_slug}_{counter}"
                counter += 1

            # Add the new record following your existing format
            self.custom_info[slug] = {
                "topic": topic.strip(),
            ...
            self.save_custom_info()
            print(f"✅ Custom info added successfully under key: {slug}")
        ...
        try:
            ...
            for row in cursor.fetchall():
                try:
                    ...
                    user_msg = sanitize_text(row[0]) if row[0] else ""
                    bot_msg = sanitize_text(row[1]) if row[1] else ""
                
                    # Only add if both messages are valid
                    ...
                    # Skip corrupted rows
                    print(f"⚠️ Skipping corrupted conversation row: {row_error}")
                    continue
...
async def chat(request: Request):  # This function handles what happens when a question is asked
    ...
    try:  # Try to process the chat safely
        ...
        chunks_found = len(docs)  # Count how many parts were found
        
        if time.time() - start_time > 18:  # Check again if it took too long
        ...
        prompt_parts = [  # This list builds the full instruction for the AI
            ...
            "",
            f"Current Question: {clean_query}",  # Include the cleaned-up question
            "",
            "Context:",
            clean_context,  # Include the related handbook info
            "",
            "Direct Answer:"  # This tells the model to start answering here
        ]
        clean_prompt = sanitize_text("\n".join(prompt_parts))  # Combine everything into one clean text
        if len(clean_prompt) > 30000:  # If it’s too long
            clean_prompt = clean_prompt[:30000] + "..."  # Cut it off safely
        
        # Step 4: Run the model and track it with LangSmith
        response_text, run_id = await run_chat_pipeline(  # Ask the AI to generate the answer
            clean_query, clean_context, clean_prompt, executor, model
        )

        if run_id:  # If there’s an ID from the AI’s run
            last_run_ids[session_id] = run_id  # Save it under this chat session
            print(f"🔗 Stored run_id {run_id} for session {session_id}")  # Show in console
        
        # Step 5: Clean and finalize the model's answer
        raw_answer = response_text  # Get the AI’s raw answer
        answer = sanitize_text(raw_answer)  # Clean it up
        if not answer:  # If the AI gave no clear answer
            answer = "I couldn't generate a proper response. Please try rephrasing your question."  # Default message
        print(f"✅ Answer generated successfully ({len(answer)} characters)")  # Confirm it worked
        
        # Save question and answer to memory
        memory.add_conversation(session_id, query, answer, clean_context[:200])  # Save the conversation for reference
        
        # Step 6: Check if the response was complete or unclear
        answered = True  # Assume the answer is good
        unclear_phrases = [  # Phrases that suggest the answer wasn’t clear or specific
        ...
        if not answer.strip() or any(p in answer.lower() for p in unclear_phrases):  # If empty or unclear
            answered = False  # Mark as unanswered
        
        # Step 7: Log everything
        await asyncio.get_event_loop().run_in_executor(  # Save what happened in the background
            executor, log_query, query, answer, answered, chunks_found
        )
        
        return {"answer": answer, "session_id": session_id}  # Send the final answer back to the student
        ...
        try:  # Try to still record the error
            ...
            memory.add_conversation(session_id, query, error_msg)  # Save the failed attempt
            await asyncio.get_event_loop().run_in_executor(
                executor, log_query, query, error_msg, False, 0
            )
        except:
            pass  # Skip if even the error logging fails
